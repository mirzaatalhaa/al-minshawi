...
type YouTubePlayer = { playVideo(): void; pauseVideo(): void; loadVideoById(id: string): void; seekTo(seconds: number, allowSeekAhead: boolean): void; getCurrentTime(): number; getDuration(): number; destroy(): void };
...
function YoutubeDisc({ playing, hostRef, configured }: { playing: boolean; hostRef: React.RefObject<HTMLDivElement | null>; configured: boolean }) { return <div className="vinyl-spin relative h-full w-full overflow-hidden rounded-full bg-black/60" data-playing={playing}><div ref={hostRef} className="youtube-viewport absolute inset-0" />{!configured && <span className="absolute inset-0 grid place-items-center bg-black/45 px-3 text-center text-[8px] leading-tight text-white/60">Add an authorized<br />YouTube video ID</span>}<span className="pointer-events-none absolute left-1/2 top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-black/70 ring-2 ring-white/40" /></div> }

function Controls({ playing, previous, toggle, next, mobile = false }: { playing: boolean; previous(): void; toggle(): void; next(): void; mobile?: boolean }) { return <div className={`flex items-center ${mobile ? "gap-2" : "gap-1"}`}><button onClick={previous} className="grid h-11 w-11 place-items-center rounded-full text-white/80 transition hover:bg-white/10 hover:text-white" aria-label="Previous Surah"><SkipIcon /></button><button onClick={toggle} className={mobile ? "grid h-[52px] w-[52px] place-items-center rounded-full bg-gradient-to-b from-accent to-[#b96f45] text-black shadow-[0_7px_20px_rgba(231,165,109,.46)] ring-1 ring-white/25" : "grid h-11 w-11 place-items-center rounded-full bg-white text-black shadow-lg transition hover:scale-105"} aria-label={playing ? "Pause recitation" : "Play recitation"}><span className="h-5 w-5"><PlayIcon playing={playing} /></span></button><button onClick={next} className="grid h-11 w-11 place-items-center rounded-full text-white/80 transition hover:bg-white/10 hover:text-white" aria-label="Next Surah"><SkipIcon next /></button></div> }

function DesktopPlayer({ track: item, playing, current, duration, seek, hostRef, previous, toggle, next }: PlayerViewProps) { return <section className={`hidden sm:flex fixed bottom-[max(1rem,env(safe-area-inset-bottom))] left-[max(1rem,env(safe-area-inset-left))] right-[max(1rem,env(safe-area-inset-right))] z-10 mx-auto max-w-xl items-center gap-4 rounded-full p-3 pr-5 ${glass}`}><div className="h-20 w-20 shrink-0"><YoutubeDisc playing={playing} hostRef={hostRef} configured={isConfigured(item.videoId)} /></div><div className="min-w-0 flex-1"><p className="truncate text-[15px] font-semibold">{item.surahName}</p><p className="truncate text-[12.5px] text-white/70">{item.reciter}</p><SeekBar current={current} duration={duration} seek={seek} /><div className="-mt-0.5 flex justify-between text-[10.5px] tabular-nums text-white/55"><span>{formatTime(current)}</span><span>{formatTime(duration)}</span></div></div><Controls playing={playing} previous={previous} toggle={toggle} next={next} /></section> }

function MobilePlayer({ track: item, playing, current, duration, seek, hostRef, previous, toggle, next }: PlayerViewProps) { return <section className={`sm:hidden fixed bottom-[max(1rem,env(safe-area-inset-bottom))] left-[max(1rem,env(safe-area-inset-left))] right-[max(1rem,env(safe-area-inset-right))] z-10 rounded-[26px] p-4 ${glass}`}><div className="flex min-w-0 items-center gap-3"><div className="h-16 w-16 shrink-0"><YoutubeDisc playing={playing} hostRef={hostRef} configured={isConfigured(item.videoId)} /></div><div className="min-w-0"><p lang="ar" dir="rtl" className="truncate font-arabic text-[17px] font-medium">{item.arabicName}</p><p className="truncate text-[12.5px] text-white/70">{item.reciter}</p></div></div><SeekBar current={current} duration={duration} seek={seek} /><div className="flex items-center justify-between"><span className="w-16 text-[10.5px] tabular-nums text-white/55">{formatTime(current)}</span><Controls mobile playing={playing} previous={previous} toggle={toggle} next={next} /><span className="w-16 text-right text-[10.5px] tabular-nums text-white/55">{formatTime(duration)}</span></div></section> }
...
export function RecitationStation() {
  ...
  const hostRef = useRef<HTMLDivElement>(null); const playerRef = useRef<YouTubePlayer | null>(null); const intendedPlay = useRef(false); const item = playlists[playlistIndex].tracks[trackIndex]; const itemRef = useRef(item); itemRef.current = item;
  ...
  const init = useCallback(() => { if (!hostRef.current || playerRef.current || !window.YT?.Player) return; playerRef.current = new window.YT.Player(hostRef.current, { videoId: isConfigured(itemRef.current.videoId) ? itemRef.current.videoId : undefined, playerVars: { controls: 1, rel: 0, playsinline: 1, modestbranding: 1 }, events: { onStateChange: (event: { data: number }) => { const state = window.YT?.PlayerState; if (!state) return; setPlaying(event.data === state.PLAYING); if (event.data === state.ENDED) advance(); }, onError: (event: { data: number }) => { const failed = itemRef.current; track("youtube_error", { code: event.data, videoId: failed.videoId, surahNumber: failed.surahNumber, surahName: failed.surahName }); advance(); } } }); }, [advance]);
  ...
  useEffect(() => { if (playerRef.current) { playerRef.current.destroy(); playerRef.current = null; } init(); }, [desktop, init]);
  useEffect(() => () => { playerRef.current?.destroy(); playerRef.current = null; }, []);
  useEffect(() => { const timer = window.setInterval(() => { const player = playerRef.current; if (!player) return; setCurrent(player.getCurrentTime() || 0); setDuration(player.getDuration() || itemRef.current.duration); }, 300); return () => window.clearInterval(timer); }, []);
  useEffect(() => { setCurrent(0); setDuration(item.duration); if (!isConfigured(item.videoId)) return; playerRef.current?.loadVideoById(item.videoId); if (!intendedPlay.current) playerRef.current?.pauseVideo(); }, [item]);
  const toggle = () => { intendedPlay.current = !playing; if (!playerRef.current) init(); if (playing) playerRef.current?.pauseVideo(); else playerRef.current?.playVideo(); };
  const seek = (amount: number) => { if (!duration || !isConfigured(item.videoId)) return; playerRef.current?.seekTo(duration * amount, true); setCurrent(duration * amount); };
  ...
  const props: PlayerViewProps = { track: item, playing, current, duration, seek, hostRef, previous: () => advance(-1), toggle, next: () => advance(1) };
